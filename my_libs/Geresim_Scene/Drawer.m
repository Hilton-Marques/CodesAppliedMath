classdef Drawer < handle
    ...
    methods
        ...
        function pts = showBB(this,bb,alpha)
            ...
            this.drawTopo(pts,0.75*[1,1,1],alpha);
        end
        function setBB(this, bb)
            %bb = this.getBB(coords);
            this.m_bb = bb;
            axis(bb);
        end
        function walkTrough(this, new_bb)
            t = 20;
            h = linspace(0,1,t);
            for h_i = h
                bb_i = this.m_bb + (new_bb - this.m_bb)*h_i;
                axis(bb_i);
                this.get();                
            end
            this.setBB(new_bb);
        ...
        function update(this, delta)
            ...
            this.angleInit_az = this.angleInit_az + delta;
        ...
        function save(this,filename, freq)
            if nargin == 1
            ...
            for idx = 1:length(this.im)
                ...
                elseif (idx > 1) && (idx < length(this.im))
                    imwrite(A,map,filename,'gif','WriteMode','append','DelayTime',freq);
                else
                    imwrite(A,map,filename,'gif','WriteMode','append','DelayTime',180);
                end
            end
        end
        function exportFrame(~,filename)
            exportgraphics(gca,strcat(filename,'.jpeg'),'Resolution',300);
        end
    end
    methods (Static)
        ...
        function h = drawRay(o,d)
            lam = 1000000;
            p2 = o + lam*d;
            p1 = o;
            h = line([p1(1),p2(1)],[p1(2),p2(2)],[p1(3),p2(3)],'linewidth',2,'color','black');
        end
