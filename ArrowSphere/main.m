...
arrow(p0, p0 + v, 'color', red ,'tipWidth', 0.060,'stemWidth', 0.03);
text(p1(1), p1(2), p1(3), "$\mathbf{v}$", 'Interpreter','latex');
...
radius = norm(v);
[X,Y,Z] = sphere(150);
s = surf(radius*X + origin(1), ...
				 radius*Y + origin(2), ...
				 radius*Z + origin(3), ...
				 'EdgeAlpha',0.0,'FaceAlpha',0.2);
colormap('gray');

% Export
exportgraphics(gcf,"arrow.png","Resolution",300);
