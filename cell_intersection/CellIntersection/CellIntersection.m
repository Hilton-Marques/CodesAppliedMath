classdef CellIntersection < handle
    properties
        ...
        m_convex_B
        m_threshold
        m_threshold_volume_abs = 1000000
        m_p % interior point
        m_drawer Drawer
        m_gjk_obj GJK
    ...
    methods
        ...
        function volume = compute(this)
            ...
            this.m_convex_A = this.projCoords(this.m_cell_A,this.m_planes_A);
            this.m_convex_B = this.projCoords(this.m_cell_B,this.m_planes_B);

            this.m_drawer.showConvexApproximation();

            this.m_convex_A = ConvexCell(this.m_convex_A);
            this.m_convex_B = ConvexCell(this.m_convex_B);
        
            this.m_gjk_obj = GJK(this.m_convex_A,this.m_convex_B);
            [isIntersecting, this.m_p] = this.m_gjk_obj.compute([this.m_planes_A;this.m_planes_B]);
            %isIntersecting = true;
            ...
            if isIntersecting
                ...
                this.m_drawer.showChildren(new_cells_A , new_cells_B);
        ...
        function new_cells = recSubdivide(this, pts, new_cells)
            ...
            if (err_concave < this.m_threshold)  
                new_cells{end+1} = pts;
            else
        ...
        function volume = getIntersectionVolume(this, planes, center)
            ...
            trisurf(k2,dualPoints(:,1),dualPoints(:,2),dualPoints(:,3),'FaceColor','magenta','FaceAlpha',1.0,'EdgeAlpha',1.0);
        end
    end
    methods (Static)
        %geometry utils
